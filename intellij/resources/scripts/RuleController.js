    ...
    var RuleController = (function () {
        ...
        RuleController.prototype.openRequestedPage = function (hash) {
            ...
            this.getContentsForVersion(requestedVersion, function () {
                ...
                if (!hash.ruleId) {
                    ...
                    document.title = 'SonarLint for IntelliJ IDEA - Version ' + hash.version;
                }
                else {
                    _this.renderRulePage(hash);
                    _this.fixRuleLinks(hash);
                    document.title = 'SonarLint for IntelliJ IDEA - Rule ' + hash.ruleId;
                }
            });
        };
        ...
        RuleController.prototype.getHash = function (input) {
            ...
            if (parsedHash.language) {
                if (parsedHash.language == 'java') {
                    hash.language = Language.Java;
                }
                else if (parsedHash.language == 'js') {
                    hash.language = Language.JavaScript;
                }
                else if (parsedHash.language == 'php') {
                    hash.language = Language.PHP;
                }
                else {
                    hash.language = Language.All;
                }
            }
            return hash;
        };
        ...
        RuleController.prototype.changeHash = function (hash) {
            ...
            if (hash.language) {
                if (hash.language == Language.Java) {
                    newHash += '&language=java';
                }
                else if (hash.language == Language.JavaScript) {
                    newHash += '&language=js';
                }
                else if (hash.language == Language.PHP) {
                    newHash += '&language=php';
                }
            }
            return newHash;
        };
                        ...
                        for (var i = 0; i < rules.length; i++) {
                            ...
                            for (var language in r.Data) {
                                meta.push({
                                    ...
                                    Language: language == 'java' ? Language.Java : (language == 'js' ? Language.JavaScript : Language.PHP)
                                });
                            }
                            r.Data = meta;
                        }
